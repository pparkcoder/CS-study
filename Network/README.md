...
- HTTP는 평문(암호화되지 않고 전송 또는 저장된 데이터) 통신이기 때문에 **도청이 가능**
...
- 현재 네트워크 시스템의 기반이 된 모델이며, 다양한 시스템의 연결은 이 계층 모델을 기반으로 통신
...
2. Sliding Window 
   ...
   - 0,1,2,3,4,5,6 -> 0,1 전송 -> 0,1에 대한 ACK 수신 -> 2,3,4,5,6,7,8 -> 2,3 전송 -> . . .
...
2. Slow Start
   ...
   - Slow Start는 AIMD와 마찬가지로 패킷을 하나식 보내는 것부터 시작, **패킷이 문제 없이 도착하면 각각의 ACK 마다 Window Size를 1씩 늘림**. 즉 한 주기가 지나면 Window Size 는 2배
...
- **비연결형 서비스**를 지원하는 **전송 계층** 프로토콜
- 전이중, 점대점 방식
- **전송 순서를 보장하지 않음**
- **흐름제어, 혼잡제어, 오류제어를 하지 않으므로 <u>손상된 세그멘트에 대한 재전송을 하지 않음</u>**
- 이로 인해 속도가 빠르며 네트워크 부하가 적다는 장점이 있지만, 신뢰성 있는 데이터 전송을 보장하지 못함
- **신뢰성보다는 연속성이 중요한 서비스에 사용 (스트리밍, 음성, 멀티미디어)**
- 헤더의 Cheksum 필드를 통해 최소한의 오류만 검출
- ***헤더의 크기가 8B로 굉장히 작고, 옵션 필드가 따로 존재하지 않아 옵션의 제한이 없음***
- MTU사이즈에 맞게 데이터를 분리해서 보낼 때, ***데이터를 의미있는 단위(데이터 그램)로 분리해서 전송***
- RTP(Real Time Protocol), Multicast, DNS 등에서 사용
...
- 즉 **여러 서버에게 균등하게 트래픽을 분산시켜 주는 것**
- 모든 사람에 대한 트래픽을 감당하기엔 1대의 서버로는 부족. **대응 방안으로 하드웨어의 성능을 올리거나(Scale-up) 여러 대의 서버가 나눠서 일하도록 만드는 것(Scale-out)**
- Scale-up : 서버가 더 빠르게 동작하기 위해 하드웨어의 성능을 올리는 방법
...
- Least Connections : 연결 갯수가 가장 적은 서버 선택(트래픽으로 인해 세션이 길어지는 경우 권장됨)
...
- URI는 인터넷의 주소 같은 것으로 정보 리소스를 유일하게 식별하고 위치 지정 가능
...
- URN은 URL의 한계로 인해 나오게 됨
- 만약 URL이 바뀌었다면 변경 전 URL로 접근했을 때 페이지를 찾을 수 없음. ***이런 문제를 해결하기 위해 객체의 위치와 상관없이 실제 객체를 가리키는 이름을 사용하기 위해 등장***
- URN은 아직 표준이 되지 않아 접할 기회가 별로 없음
...
### in 브라우저
	...
	1. url에 입력된 값을 브라우저 내부에서 결정된 규칙에 따라 그 의미를 조사
 	2. 조사된 의미에 따라 HTTP Request 메세지를 만듬
 	3. 만들어진 메세지를 웹 서버로 전송

이때 만들어진 메세지 전송은 브라우저가 직접하는 것이 아님. **브라우저는 메세지를 네트워크에 송출하는 기능이 없으므로 OS에 의뢰하여 메세지를 전달**.단 OS에 송신을 의뢰할 때는 도메인명이 아니라 ip주소로 메세지를 받을 상대를 지정해야 하는데, 이 과정에서 DNS서버를 조회해야 함.

<BR>

### in 프로토콜 스택, LAN 어댑터

	1. 프로토콜 스택(운영체제에 내장된 네트워크 제어용 S/W)이 브라우저로부터 메세지를 받음
 	2. 브라우저로부터 받은 메세지를 패킷 속에 저장
 	3. 수신처 주소 등의 제어 정보를 덧붙임
 	4. 패킷을 LAN 어댑터에 넘김
 	5. LAN 어댑터는 다음 Hop의 MAC 주소를 붙인 프레임을 전기신호로 변환
 	6. 신호를 LAN 케이블에 송출

**프로토콜 스택은 통신 중 오류가 발생했을 때, 이 제어 정보를 사용하여 고쳐 보내거나, 각종 상황을 조절하는 등 다양한 역할을 하게 됨**.

네트워크 세계에서는 비서가 있어서 우리가 비서에게 물건만 건네주면, 받는 사람의 주소와 각종 유의사항을 써줌. 여기서는 프로토콜 스택이 비서의 역할
...
### in 액세스 회선, 프로바이더
	...
	1. 패킷은 인터넷의 입구에 있는 액세스 회선(통신 회선)에 의해 POP(Point Of Presence, 통신사용 라우터)까지 운반됨
 	2. POP를 거쳐 인터넷의 핵심부로 들어가게 됨
...
굳이 서버까지 가지 않아도 되는 경우를 골라냄. 액세스한 페이지의 데이터가 캐시서버에 있으면 웹 서버에 의뢰하지 않고 바로 그 값을 읽을 수 있음. 페이지의 데이터 중에 다시 이용할 수 있는 것이 있으면 캐시 서버에 저장됨
...
### in 웹 서버
	...
	1. 패킷이 물리적인 웹 서버에 도착하면 웹 서버의 프로토콜 스택은 패킷을 추출하여 메세지를 복원하고 웹 서버 애플리케이션에 넘김
 	2. 메세지를 받은 웹 서버 애플리케이션은 요청 메세지에 따른 데이터를 응답 메세지에 넣어 클라이언트로 회송
